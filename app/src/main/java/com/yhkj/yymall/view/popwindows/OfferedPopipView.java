...
public class OfferedPopipView extends BasePopupWindow {
    ...
    private void getSpecList(){
        YYMallApi.getEnableSpec(getContext(),String.valueOf(mDataBean.getGoods().getId()),null,new YYMallApi.ApiResult<EnableSpecBean.DataBean>(getContext()){
            @Override
...
//        mLastSelectSpecId = id;
        ...
        YYMallApi.getEnableSpec(getContext(),String.valueOf(mDataBean.getGoods().getId()),jsonStr,new YYMallApi.ApiResult<EnableSpecBean.DataBean>(getContext()){
            @Override
            public void onError(ApiException e) {
                super.onError(e);
                showToast(e.getMessage());
