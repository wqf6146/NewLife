...
public class VideoHeaderView extends RelativeLayout implements RefreshHeader {
...
//    private TextView mHeaderText;
    ...
    private void initView(Context context, AttributeSet attrs, int defStyleAttr) {
        mRootView = LayoutInflater.from(context).inflate(R.layout.view_refresh_header_video,this);
...
//        }
    ...
    public void onPullingDown(float percent, int offset, int headHeight, int extendHeight) {
        if (percent < 1.0f){
            if (percent < 0.5f){
                float p = percent * 2;
                mOnRefreshLisiten.onPullDowning(p,offset);
            }else{
                mOnRefreshLisiten.onPullDowning(null,offset);
            }
        }
    }
...
//                mHeaderText.setText(REFRESH_HEADER_RELEASE);
    ...
    public interface OnRefreshLisiten{
        void onStartPull();
        void onBackTop();
        void onPullDowning(Float percent,int offset);
        void onPullReleasing(Float percent,int offset);
    }

    public VideoHeaderView setOnRefreshLisiten(OnRefreshLisiten onRefreshLisiten ) {
