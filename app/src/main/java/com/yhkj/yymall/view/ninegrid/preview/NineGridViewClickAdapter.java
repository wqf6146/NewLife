package com.yhkj.yymall.view.ninegrid.preview;
...
import com.yhkj.yymall.view.ninegrid.NineGridViewAdapter;
...
public class NineGridViewClickAdapter extends NineGridViewAdapter {
    ...
    protected void onImageItemClick(Context context, NineGridView nineGridView, int index, List<ImageInfo> imageInfo) {
        ...
        startPhotoActivity(context,index,imageInfo);
...
//        ((Activity) context).overridePendingTransition(0, 0);
    ...
    public void buildBannerBean( List<ImageInfo> imageInfo){
        mBannerItemBean = new ArrayList<>();
        for (int i=0;i<imageInfo.size();i++){
            mBannerItemBean.add(new BannerItemBean(Constant.BANNER.IMG,null,imageInfo.get(i).getBigImageUrl()));
        }
    }


    public  void startPhotoActivity(Context context,int index, List<ImageInfo> imageInfo) {
        buildBannerBean(imageInfo);
//        Intent intent = new Intent(context, ShopBannerActivity.class);
...
//        intent.putExtra("width", imageView.getWidth());
        ...
        intent.putExtra(Constant.BANNER.POSITION,index);
        context.startActivity(intent);
